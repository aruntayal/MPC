    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          const int NUM_OF_WAYPOINTS = ptsx.size();
          Eigen::VectorXd waypoints_xs(NUM_OF_WAYPOINTS);
          Eigen::VectorXd waypoints_ys(NUM_OF_WAYPOINTS);
          ...
          const double current_psi = 0.0 + v * (-1 * steering_angle) / Lf * dt;
          const double current_v = v + throttle * dt;
          const double current_cte = cte + v * sin(epsi) * dt;
          const double current_epsi = epsi + v * (-1 * steering_angle) / Lf * dt;
          ...
          std::cout<<"\n returned from solve  steer_value  "<<steer_value <<"    "<<throttle_value;
